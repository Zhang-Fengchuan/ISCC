function[beta,BIC,H]=SCCgai(x,y,lon,lat,options)
% Description:
...
[H]=SCC_spanning_treegai(lon,lat,p,0.1,x,y);
%%%%%%%%%%
%%%%%%%%%%
%%%%%%%%%%
...
distance=sqrt((lon-lon').^2+(lat-lat').^2);%%%%%%%%%%%%%%%%
[u,v]=size(H);%%%%%%%%%%%%%%%%%%%%
chengfa=zeros((n-1)*p,1);
    for i=1:u-p%%%%%%%%%%%%%%%%%%%%
        zhibiao=find(H(i,:)~=0);  %%%%%%%%%%%%%%%%%%%%%%
        if mod(zhibiao(1),n)~=0&&mod(zhibiao(2),n)~=0
           chengfa(i)=1/distance(mod(zhibiao(1),n),mod(zhibiao(2),n));
        elseif mod(zhibiao(1),n)==0&&mod(zhibiao(2),n)~=0
           chengfa(i)=1/distance(n,mod(zhibiao(2),n)); 
        elseif mod(zhibiao(2),n)==0&&mod(zhibiao(1),n)~=0
           chengfa(i)=1/distance(mod(zhibiao(1),n),n); 
        else
           chengfa(i)=1/distance(n,n); 
        end
    end
...
elseif options.intercept_type==1
    ...
    options.penalty_factor=ones(n*p-1,1);  %%%%%%%%%%%%%%%%5
    %options.penalty_factor(1:(n-1)*p,1)=chengfa;
